expect.extend({
  toEqualBuffer(expected: ArrayBuffer, observed: ArrayBuffer): jest.CustomMatcherResult {
    ...
    if (expected.byteLength !== observed.byteLength) {
      return {
        pass: false,
        message: () => `\`ArrayBuffer\` byte lengths do not match (${expected.byteLength} vs. ${observed.byteLength}). Expected ${expectedUint8Str()}.`,
      };
    }
    for (let i = 0; i < expected.byteLength; i++) {
      if (expectedUint8[i] !== observedUint8[i]) {
        return {
          pass: false,
          message: () => `\`ArrayBuffer\` mismatch on byte #${i}. Expected ${expectedUint8Str()}.`,
        };
      }
    }
